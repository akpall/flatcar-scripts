...
src_prepare() {
	...
	if [[ ${PV} != 9999* ]]; then
		...
		ln -sr "${WORKDIR}/erofs-utils-${EROFS_UTILS_COMMIT}" "${BUILD_DIR}"/_deps/erofs-utils-src || die
		ln -sr "${WORKDIR}/PhotonLibOS-${PHOTON_COMMIT#v}" "${BUILD_DIR}"/_deps/photon-src || die
		ln -sr "${WORKDIR}/photon-libtcmu-${TCMU_COMMIT#v}" "${BUILD_DIR}"/_deps/tcmu-src || die
