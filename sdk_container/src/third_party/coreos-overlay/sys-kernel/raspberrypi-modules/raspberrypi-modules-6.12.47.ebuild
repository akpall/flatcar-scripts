...
src_prepare() {
	...
	if [[ ! -f build/.config ]]; then
		local defconfig="$(find_config raspberrypi_defconfig-6.12)"
		local commonconfig="$(find_commonconfig)"
		elog "Building using config ${defconfig} and ${commonconfig}"
		cat "${defconfig}" "${commonconfig}" | envsubst '$MODULE_SIGNING_KEY_DIR' >> build/.config || die
	fi
